...
import { Play, CheckCircle, Clock, AlertTriangle, XCircle, Loader2 } from 'lucide-react';
...
function defaults(type: NodeType): Record<string, any> {
  if (type === 'agent') return { model: 'gpt-5.4', temperature: 0, toolAccess: 'restricted', agentCount: 1, personaMode: 'auto', personaNames: '', systemPrompt: '' };
...
export default function WorkflowsDashboard() {
  ...
  async function ensureDefaultBoard() {
    ...
    } catch (e: any) { setApiError(e?.message || 'API error'); }
  }
  ...
  async function loadWorkflow(id: string) {
    ...
    const def = typeof d.workflow.definition === 'string'
      ? JSON.parse(d.workflow.definition)
            ...
            <CardContent>
              ...
              <div className="space-y-2">
                {runs.map((r: any) => {
                  ...
                  return (
                    <div key={r.id} className="py-2 px-3 rounded-md border border-border/50 hover:bg-accent/30 transition-colors space-y-1.5">
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <Link to={`/boards/run/${r.run_id}`} className="text-xs text-primary hover:underline truncate font-mono">
                            {r.run_id?.slice(0, 12)}
                          </Link>
                          {r.result && (
                            <Badge variant="secondary" className="text-[10px] shrink-0">{r.result}</Badge>
                          )}
                        </div>
                        {r.status === 'waiting' && (
                      ...
                      <div className="flex items-center gap-3 text-[10px] text-muted-foreground pl-6">
                        <span>Started {formatTimestamp(r.started_at)}</span>
                        <span>Completed {formatTimestamp(r.completed_at)}</span>
                        <span>Duration {formatDuration(r.duration_ms)}</span>
                      </div>
                    </div>
                  );
                })}
