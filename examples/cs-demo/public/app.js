...
async function runScenario() {
  ...
  $("#scenario-content").innerHTML = '<div class="progress-indicator"><div class="progress-step active">Generating scenario...</div></div>';
  $("#response-content").innerHTML = '<div class="progress-indicator"><div class="progress-step">Waiting for scenario...</div></div>';
  $("#decision-content").innerHTML = '<div class="progress-indicator"><div class="progress-step">Waiting for response...</div></div>';
...
async function stageReveal(data) {
  ...
  $("#scenario-content").innerHTML = `
    <div class="email-envelope fade-in">
      <div class="email-header">
        <div class="email-avatar email-avatar-customer">C</div>
        <div>
          <div class="email-from">Customer</div>
          <div class="email-to">To: support@company.com</div>
          <div class="email-subject">Subject: Support Request — ${escapeHtml(data.riskTier)} priority</div>
        </div>
  ...
  for (let i = 0; i < cycles.length; i++) {
    ...
    if (isInitial) {
      $("#response-content").innerHTML = '<div class="progress-indicator"><div class="progress-step active">CS agent drafting response...</div></div>';
      await delay(2000);
    } else {
      // Rewrite indicator
    ...
    responseBlock.innerHTML = `
      ...
      <div class="email-header">
        ...
        <div>
          <div class="email-from">CS Agent${!isInitial ? " (revised)" : ""}</div>
          <div class="email-to">To: customer</div>
        </div>
  ...
  let responseHtml = `
    <div class="email-envelope">
      <div class="email-header">
        <div class="email-avatar email-avatar-agent">A</div>
        <div>
          <div class="email-from">CS Agent${data.rewriteCount > 0 ? " (revised)" : ""}</div>
          <div class="email-to">To: customer</div>
        </div>
        <div class="email-time">${t}</div>
      </div>
...
function showSlackDialog(hitlMessage, runId, data) {
  ...
  if (data) {
    ...
    if (data.scenario) {
      contextHtml += `
        <div class="slack-context-label">Customer Email</div>
        <div class="slack-context-card">
          <div class="email-avatar email-avatar-customer">C</div>
          <div>
            <div class="slack-context-from">Customer</div>
    ...
    if (agentResponse) {
      contextHtml += `
        <div class="slack-context-label">Agent Response</div>
        <div class="slack-context-card">
          <div class="email-avatar email-avatar-agent">A</div>
          <div>
            <div class="slack-context-from">CS Agent${data.rewriteCount > 0 ? " (revised)" : ""}</div>
...
async function resetAll() {
  ...
  $("#btn-run").disabled = true;
  $("#btn-run").textContent = "Run";
  $("#scenario-content").innerHTML = '<p class="placeholder">Select a risk tier and click Run.</p>';
  $("#response-content").innerHTML = '<p class="placeholder">Agent response will appear here.</p>';
  $("#decision-content").innerHTML = '<p class="placeholder">Guard evaluation results will appear here.</p>';
  $("#reputation-content").innerHTML = '<p class="placeholder">Run a scenario to see reputation.</p>';
