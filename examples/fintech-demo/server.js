...
import { getAgentResponse, rewriteAgentResponse } from "./lib/agent.js";
...
function initGuardRep() {
  const personas = ["security-analyst", "compliance-officer", "operations-engineer", "bsa-aml-reviewer", "transaction-risk-analyst"];
  guardReputation = {};
...
app.post("/api/run", async (req, res) => {
  try {
    ...
    emit("job.submitted", runId, { step: "agent-eval" });
    const csResponse = await getAgentResponse(scenario, model);
    emit("job.submitted", runId, { step: "agent-complete" });
...
async function handleRewrite(run, mdl) {
  ...
  while (rewriteCount < MAX_REWRITES) {
    ...
    currentResponse = await rewriteAgentResponse(run.scenario, currentResponse, feedback, mdl);
...
app.listen(PORT, () => {
  console.log(`Fintech Transaction Guard Demo running at http://localhost:${PORT}`);
});
