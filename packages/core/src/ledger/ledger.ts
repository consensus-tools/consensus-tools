import type { ConsensusToolsConfig, LedgerEntry, StorageState } from "@consensus-tools/schemas";
import type { IStorage } from "@consensus-tools/storage";
import { computeBalances, ensureNonNegative, getBalance } from "./rules.js";
...
export class LedgerEngine {
  ...
  private async appendEntry(entry: LedgerEntry): Promise<LedgerEntry> {
    ...
    this.logger?.info?.(
      `consensus-tools: ledger entry (${entry.type} ${entry.amount} agent=${entry.agentId}${entry.jobId ? ` job=${entry.jobId}` : ""})`,
    );
    if (this.config.local.ledger.balancesMode === "override") {
      await this.applyConfigBalances(this.config.local.ledger.balances, "override");
    }
